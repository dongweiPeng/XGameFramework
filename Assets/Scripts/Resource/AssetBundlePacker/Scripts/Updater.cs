    ...
    public class Updater : MonoBehaviour
    {
                                                ...
                                                , ResourcesManifest new_resourcesmanifest)
        {
            ...
            while (itr.MoveNext())
            {
                ...
                if (mask == new_version_bit)
                    add_files.Add(name);
                else if (mask == both_bit)
                    both_files.Add(name);
                else if (mask == old_version_bit)
            ...
            if (delete_files.Count > 0)
            {
                foreach(var v in delete_files)
                {
                    Debug.Log(string.Format("删除文件>{0}", v));
                }
            }

            if (add_files.Count > 0)
            {
                foreach (var v in add_files)
                {
                    Debug.Log(string.Format("新增文件>{0}", v));
                }
            }

            if (both_files.Count > 0)
            {
                foreach (var v in both_files)
                {
                    Debug.Log(string.Format("两者都有文件>{0}", v));
                }
            }
        ...
        IEnumerator StartCopyCacheFile()
        {
            ...
            for (int i = 0; i < Common.MAIN_CONFIG_NAME_ARRAY.Length; ++i)
            {
                ...
                Debug.Log(string.Format("{0}覆盖文件{1}", str, dest));
